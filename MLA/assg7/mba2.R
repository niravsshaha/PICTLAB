library(readxl)
#dataset = read.csv("Online Retail.xlsx", na.strings = c("","NA"))
dataset = read_excel("Online Retail.xlsx")
attach(dataset)
...
items = ddply(dataset,c("InvoiceNo"), function(x)paste(x$Description, collapse = ","))
head(items)

write.csv(items,"Items_List.csv",quote=FALSE, row.names = TRUE)

#creating the baskets
baskets = read.csv("Items_List.csv", format='basket',sep=",")
summary(baskets)
